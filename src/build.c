...
static const char *exit_names[] = { "north", "south", "east", "west", "northeast", "northwest", "southeast", "southwest", "up", "down", 0 };
static const char *opposites[] =  { "south", "north", "west", "east", "southwest", "southeast", "northwest", "northeast", "down", "up", 0 };
...
void cmd_eset( D_MOBILE *dMob, char *arg )
{
   ...
   D_EXIT *exit = get_exit_by_name( dMob->room, dir ), *bexit;
   if( exit == NULL )
   {
   ...
   if( is_bexit == TRUE && ( bexit = get_exit_by_name( exit->to_room, exit->farside_name ) ) != NULL )
   {
      char buf[MAX_STRING_LENGTH];
      snprintf( buf, MAX_STRING_LENGTH, "%s %s %s", exit->farside_name, what, arg );
      D_ROOM *here = dMob->room;
      mob_to_room( dMob, exit->to_room );
      cmd_eset( dMob, buf );
      mob_to_room( dMob, here );
   }
   return;
}
...
void cmd_redit( D_MOBILE *dMob, char *arg )
{
   ...
   else if( !strcasecmp( arg1, "sector" ) || !strcasecmp( arg1, "sectortype" ) )
   {
      int i;
      char sector[MAX_STRING_LENGTH];
      arg = one_arg( arg, sector );
      if( sector[0] == '\0' )
      {
         text_to_mobile_j( dMob, "error", "Set the room's sector type to what?" );
         return;
      }
      for( i = 0; i < MAX_SECTOR; i++ )
      {
         if( !strcasecmp( sector, sector_types[i] ) )
            break;
      }
      if( i == MAX_SECTOR )
      {
         text_to_mobile_j( dMob, "error", "Invalid sector type." );
         return;
      }
      dMob->room->sectortype = i;
      text_to_mobile_j( dMob, "text", "Room sector type set to %s.", sector_types[i] );
   }
   ...
   else if( !strcasecmp( arg1, "exit" ) || !strcasecmp( arg1, "bexit" ) )
   {
      char dir[MAX_STRING_LENGTH], buf[MAX_STRING_LENGTH];
      int vnum = 0, i = 0;
      arg = one_arg( arg, dir );
      vnum = atoi( arg );
      D_ROOM *room;
      ...
      for( i = 0; i < 10; i++ )
      {
         if( !strcasecmp( dir, exit_abbr[i] ) || !strcasecmp( dir, exit_names[i] ) )
            break;
      }
      if( i == 10 )
      {
      ...
      exit->name = strdup( exit_names[i] );
      exit->farside_name = strdup( opposites[i] );
      exit->to_room = room;
      exit->to_vnum = vnum;
      if( toupper( arg1[0] ) == 'B' ) //command was bexit, so we create both exits
      {
         D_ROOM *here = dMob->room;
         mob_to_room( dMob, room );
         snprintf( buf, MAX_STRING_LENGTH, "exit %s %i", exit->farside_name, here->vnum );
         cmd_redit( dMob, buf );
         mob_to_room( dMob, here );
      }
